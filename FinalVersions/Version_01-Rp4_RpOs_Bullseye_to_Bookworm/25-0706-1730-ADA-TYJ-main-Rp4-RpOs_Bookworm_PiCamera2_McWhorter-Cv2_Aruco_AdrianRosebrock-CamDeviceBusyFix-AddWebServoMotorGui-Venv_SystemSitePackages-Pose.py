    ...
    while True:
        ...
        if len(corners) > 0:
            ...
            for i, (markerCorner, markerID) in enumerate(zip(corners, ids)):
                ...
                # --- Display Distance from Pose ---
                # The Z-value of the translation vector (tvecs) is the distance in meters
                distance = tvecs[i][0][2]

                # DEBUG 3: Show the info for each specific marker being processed
                print(f"[INFO] Marker ID: {markerID} -> Distance: {distance:.2f}m")

                cv2.putText(frame, f"ID: {markerID} Dist: {distance:.2f}m",
